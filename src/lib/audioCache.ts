...
/**
 * For Drive-backed URLs (/api/media/audio/...): return blob: URL from LRU cache (max 10),
 * or fetch once, store, and return blob: URL. For other URLs returns as-is.
 */
export async function resolveAudioSource(trackId: string, streamUrl: string): Promise<string> {
  const isDriveProxy =
    streamUrl.startsWith('/api/media/audio/') ||
    streamUrl.includes('/api/media/audio/');
  if (!isDriveProxy) {
    return streamUrl;
  }
